...
function writeWorkings(ws: ExcelJS.Worksheet, company: CompanyProfile): void {
  ws.state = 'veryHidden';
  ws.getCell('A1').value = 'Fiscal Year Data';
  ws.getCell('B1').value = company.fiscalYear;
  ws.getCell('A2').value = 'Generated by NFRS Reporter';
  ws.getCell('A3').value = new Date().toISOString();
}
...
export function writeChangesInEquity(ws: ExcelJS.Worksheet, ce: ChangesInEquity, company: CompanyProfile): void {
  ...
  const ceRows: [string, number, number, number, number, number][] = [
    ['Opening Balance (1 Shrawan)', ce.openingShareCapital, ce.openingSharePremium, ce.openingGeneralReserve, ce.openingRetainedEarnings, ce.openingTotal],
    ['Profit for the Year', 0, 0, 0, ce.addProfitForYear, ce.addProfitForYear],
    ['Issue of Share Capital', ce.addNewShareCapital, ce.addSharePremiumOnNewIssue, 0, 0, ce.addNewShareCapital + ce.addSharePremiumOnNewIssue],
    ['Transfer to General Reserve', 0, 0, ce.addTransferToReserve, -ce.addTransferToReserve, 0],
    ['Dividends Paid', 0, 0, 0, -ce.lessDividendPaid, -ce.lessDividendPaid],
    ['Closing Balance (31 Ashadh)', ce.closingShareCapital, ce.closingSharePremium, ce.closingGeneralReserve, ce.closingRetainedEarnings, ce.closingTotal],
  ];
...
function writeGenericNoteRecord(ws: ExcelJS.Worksheet, title: string, data: Record<string, { cy: number; py: number }>): void {
...
export async function generateNFRSWorkbook(params: {
  ...
  writeWorkings(addSheet('Workings', COLORS.LIGHT_GRAY), company);
