...
function DataRow({
  label, note, cy, py, rl, indent = false,
}: {
  label:   string;
  note?:   string;
  ...
  return (
    <tr>
      <td className={`${indent ? 'pl-8' : ''} text-[13px] text-slate-700`}>{label}</td>
      {/* item 102: wider note column w-[7%] with better padding */}
      <td className="text-center text-xs w-[7%] px-1.5 py-1">
...
// item 108: slightly more breathing room spacer
function BlankRow() {
  return <tr className="spacer-row"><td colSpan={4} /></tr>;
}

function BalanceSheetView({
  data, company, previousYear,
  ...
  return (
    <div className="statement-page max-w-4xl mx-auto">
      {/* item 99: label updated to "Print / Export PDF" */}
      ...
      <div className="statement-header">
        {/* item 100: company name at 1.125rem (18px) */}
        <p className="statement-company-name" style={{ fontSize: '1.125rem' }}>
          {company.companyName}
        </p>
        {/* item 100: statement title at 0.875rem (14px) */}
        ...
        {/* item 100: rounding notice italic */}
      ...
      <table className="fin-table w-full mt-4">
        <colgroup>
          <col style={{ width: '55%' }} />
          <col style={{ width: '7%' }} />   {/* item 102: widened note column */}
          <col style={{ width: '19%' }} />
          <col style={{ width: '19%' }} />
        </colgroup>
        <thead>
      ...
      {/* item 109 + 110 + 111: Signature block */}
      <div className="border-t border-slate-200 mt-6 pt-4 signature-block">
        <p className="text-xs text-slate-500 mb-4">For and on behalf of the Board of Directors</p>
        {/* item 111: sm:grid-cols-1 lg:grid-cols-3 responsive */}
        <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-3 gap-6 text-xs text-slate-600">
          {[
            ...
            <div key={sig.role} className="flex flex-col items-start">
              {/* item 109: h-10 space then border BELOW (not above) the name */}
        ...
        <div className="mt-6 flex items-start justify-between flex-wrap gap-4">
          <div className="flex flex-col items-start">
            ...
            {/* item 110: digital preview placeholder for auditor signature */}
            <div className="w-36 h-16 bg-slate-50 rounded border border-slate-200 flex items-center justify-center mb-2">
